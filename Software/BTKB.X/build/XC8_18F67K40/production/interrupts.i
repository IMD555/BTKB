...
# 10 "C:\\Program Files (x86)\\Microchip\\xc8\\v2.00\\pic\\include\\c99\\string.h" 3
...
void __attribute__((picinterrupt("low_priority"))) low_isr(void)
{
 ...
 else if (PIE3bits.TX2IE && PIR3bits.TX2IF)
 {
  if (!RF2)
  {
   if ((bufU2_main.readIndex != bufU2_main.writeIndex))SendCharBT(bufRead(&bufU2_main));

   else if ((bufU2_intH.readIndex != bufU2_intH.writeIndex)) SendCharBT(bufRead(&bufU2_intH));
 ...
 else if (PIE4bits.TX5IE && PIR4bits.TX5IF)
 {
  if ((!RF6&&!!RF7&&!RF5))
  {
   if (RF0)
   {
    if ((bufU5_main.readIndex != bufU5_main.writeIndex))SendCharUSB(bufRead(&bufU5_main));

    else if ((bufU5_intH.readIndex != bufU5_intH.writeIndex))SendCharUSB(bufRead(&bufU5_intH));
